...
1. Download the 2040--2069 mid-century monthly BCSD VIC data for Montana and the HUC 6 hydrological basins it overlaps.
...
bcsd_swe_midcentury <- 
  ...
  purrr::map_chr(function(var){
    thredds::tds_ncss_download(ncss_url = "https://cida.usgs.gov/thredds/ncss/BCSD_mon_VIC/dataset.html",
                               out_file = stringr::str_c(data_out,"/raw_data/",var,"_2040-2069.nc"),
...
bcsd_swe_1971_2000 <- 
  ...
  purrr::map_chr(function(var){
    thredds::tds_ncss_download(ncss_url = "https://cida.usgs.gov/thredds/ncss/BCSD_mon_VIC/dataset.html",
                               out_file = stringr::str_c(data_out,"/raw_data/",var,"_1971-2000.nc"),
                               bbox = mcor::mt_watersheds_simple %>%
                                 dplyr::filter(`Hydrologic Unit` == 6) %>%
                                 sf::st_bbox() %>%
                                 sf::st_as_sfc() %>%
...
bcsd_swe_1981_2010 <- 
  ...
  purrr::map_chr(function(var){
    thredds::tds_ncss_download(ncss_url = "https://cida.usgs.gov/thredds/ncss/BCSD_mon_VIC/dataset.html",
                               out_file = stringr::str_c(data_out,"/raw_data/",var,"_1981-2010.nc"),
                               bbox = mcor::mt_watersheds_simple %>%
                                 dplyr::filter(`Hydrologic Unit` == 6) %>%
                                 sf::st_bbox() %>%
                                 sf::st_as_sfc() %>%
...
Here is a panel of maps of the 1981--2010 median April 1 SWE (in.) compared to the mid-century median April 1 SWE (in.), with HUC 6 watersheds overlaid.
...
pal <- colorRampPalette(rev(paste0(RColorBrewer::brewer.pal(9,"Blues"),"80")), alpha = TRUE)
...
pal <- colorRampPalette(c(paste0(RColorBrewer::brewer.pal(9,"RdBu"),"80")[1:3],paste0(RColorBrewer::brewer.pal(9,"RdBu"),"80")[7:9]), alpha = TRUE)
