...
snotel_normals_quantiles <- snotel_normals %>%
  dplyr::mutate(Date = lubridate::yday(Date) %>% 
                  as.integer() %>%
                  magrittr::subtract(273),
                Date = ifelse(Date <= 0, Date+365, Date)) %>%
  dplyr::group_by(Date,`Station Id`) %>%
  dplyr::arrange(Date,`Station Id`) %>%
...
snotel_2018_test %<>%
  ...
  dplyr::mutate(`0% NEP` = `0%` + (SWE[dowy]-`0%`[dowy]))
